...
/// The central class for configuration of all security aspects. Automatically used when not running
/// with profile `no-security`. Configures all endpoints to require authentication via access token.
/// (except the Spring Boot Actuator endpoints) Additionally it configures the use of role-based
/// authorization (via application.yml) or [KeycloakPermissionsAuthoritiesConverter] (when profile
/// "keycloak-permissions" is set).
